...
function route($route, $path_to_include){
  ...
  if($route == "/404"){
    ...
    //
    include_once dirname(__DIR__,1)."/$path_to_include";
    //
    exit();
  }  
  //$request_url = filter_var($_SERVER['REQUEST_URI'], FILTER_SANITIZE_URL);
  //
  $api_root=str_replace($_SERVER['DOCUMENT_ROOT'],'',getcwd());
  $request_url =str_replace($api_root,'',filter_var($_SERVER['REQUEST_URI'], FILTER_SANITIZE_URL));
  //
